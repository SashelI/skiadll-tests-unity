...
#pragma warning restore 0649
		...
		[SerializeField]
		private float canvasOffestZ=0.5f;
...
#pragma warning restore 0414
		...
		private void Awake()
		{
			...
			canvas = browserCanvas.GetComponent<Canvas>();
			rectTransform = browserCanvas.GetComponent<RectTransform>();
		...
		public void OnItemSelected( FileBrowserItem item, bool isDoubleClick )
		{
			...
			{
				if (selectedFileEntries.Count > 0 &&
				    (isDoubleClick || (SingleClickMode && !m_multiSelectionToggleSelectionMode)))
				{
					// Submit selection
		...
		public void Show( string initialPath, string initialFilename )
		{
			...
			browserCanvas.transform.position = mainCameraPosition + mainCameraTransform.forward * canvasOffestZ;
			browserCanvas.transform.LookAt(mainCameraTransform);
			Vector3 browserRotation = browserCanvas.transform.rotation.eulerAngles;
			browserCanvas.transform.rotation = Quaternion.Euler(0, browserRotation.y - 180, browserRotation.z);
